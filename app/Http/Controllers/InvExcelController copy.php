...
class InvExcelController extends Controller
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // where('dic_status', '!=', 'G')
        // ->orWhere('oec_status', '!=','G')
        // // ->groupBy('concerned_dept')
        // ->distinct()
        // ->get();

        // $plc_section = DB::table('tbl_plc_module_sa')
        // ->where('dic_status', '!=', 'G')
        // ->orWhere('oec_status', '!=','G')
        // ->distinct()
        // ->get();
        ...
        $plc_section = DB::table('tbl_plc_module_sa')
             ->select(DB::raw('DISTINCT concerned_dept'))
        ->where('dic_status', '!=', 'G')
                ->orWhere('oec_status', '!=','G')
...
// ->groupBy('status')
        ...
        $plc_section = collect($plc_section)->pluck('concerned_dept')->unique()->toArray();

        // return $plc_section;

        // $chris = array();

        // foreach($plc_section as $varia){
        //     array_push($chris,$varia);
        // }
    ...
    for($x = 1; $x<= 36; $x++){
        ...
        return Excel::download(new UsersExports(
            ...
            $plc_section,
            ), 'PMI FY'.$result.' PLC Audit Result.xlsx');
